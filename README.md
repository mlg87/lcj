...
| [`clusage-menubar/`](./clusage-menubar/) | **Clusage** — a macOS menu bar app showing Claude 5h / weekly / Fable usage as a compact grid of progress bars, authenticated by your claude.ai session cookie — plus a Codex column when the Codex CLI is installed: estimated cost from local session logs, and a monthly limit gauge fetched from ChatGPT's spend controls. |
